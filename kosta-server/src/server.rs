        ...
        if let Some(data) = response {
            ...
            if let Some(mut crypto) = crypto_ctx {
                ...
                if let Some(id) = auth_key_id {
                    if let Ok(mut session) = cache.get_session(id) {
                        if let Some(ref mut s) = session {
                            send_seq_no = s.send_seq_no + 1;
                            s.send_seq_no = send_seq_no;
                            msg_id_counter = s.msg_id_counter;
                            s.msg_id_counter = s.msg_id_counter.wrapping_add(1);
                            cache.update_session(id, s.clone())?;
                        }
                    }
                }
                let now = SystemTime::now().duration_since(UNIX_EPOCH).unwrap();
                ...
                if let Some(id) = auth_key_id {
                    if let Ok(mut session) = cache.get_session(id) {
                        if let Some(ref mut s) = session {
                            s.crypto = crypto.clone();
                            s.send_counter = crypto.send_counter;
                            s.recv_counter = crypto.recv_counter;
